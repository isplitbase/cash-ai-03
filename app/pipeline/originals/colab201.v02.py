...
# ==========
def libreoffice_recalc_via_convert(src_xlsx: str, out_xlsx: str, workdir: str | None = None):
    if workdir is None:
        workdir = BASE_DIR
    ...
    lo_out = run_cmd([
        "bash", "-lc",
        f'libreoffice --headless --nologo --nodefault --nofirststartwizard '
        f'--convert-to "xlsx:Calc MS Excel 2007 XML" --outdir "{workdir}" "{tmp_ods}"'
    ])

    after = set(os.listdir(workdir))
    created = list(after - before)

    # 新規作成ファイルのうち Excel を拾う（拡張子は大小文字無視）
    excel_created = [
        f for f in created
        if os.path.splitext(f)[1].lower() in (".xlsx", ".xls")
    ]

    # created に出ないケース（同名上書き等）もあるので保険で探す
    if not excel_created:
        excel_created = [
            f for f in os.listdir(workdir)
            if (("_tmp_recalc" in f) and (os.path.splitext(f)[1].lower() in (".xlsx", ".xls")))
        ]

    if len(excel_created) == 1:
        generated_xlsx = os.path.join(workdir, excel_created[0])
    elif len(excel_created) >= 2:
        excel_paths = [os.path.join(workdir, f) for f in excel_created]
        generated_xlsx = max(excel_paths, key=lambda p: os.path.getmtime(p))
    else:
        listing = "\n".join(sorted(os.listdir(workdir)))
        raise RuntimeError(
            "LibreOffice conversion back to XLSX did not produce an identifiable Excel file.\n"
...
def main():
    ...
    libreoffice_recalc_via_convert(INTERIM_EXCEL_PATH, RECALC_EXCEL_PATH, workdir=BASE_DIR)
